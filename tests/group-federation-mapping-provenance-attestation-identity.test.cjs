...
test('attestationRef changes when the signed validity window changes', () => {
  ...
  const shortLived = createFederationMappingProvenanceAttestation({
    ...common,
    validUntil: '2026-08-11T03:06:00Z',
  });
  const longerLived = createFederationMappingProvenanceAttestation({
    ...common,
    validUntil: '2026-08-11T03:11:00Z',
  });

  assert.notEqual(shortLived.validUntil, longerLived.validUntil);
  assert.notEqual(shortLived.attestationDigest, longerLived.attestationDigest);
  assert.notEqual(
    shortLived.attestationRef,
    longerLived.attestationRef,
    'attestation identity must bind the full signed validity window',
  );
});
