...
function version({ bindings = [] } = {}) {
  return {
    ...
    ...publishCapabilityVersion({
      packageId: 'local.form-submit',
      version: '1.0.0',
      integrityDigest: DIGEST,
      inputSchema: { type: 'object', required: ['payload'] },
      outputSchema: { type: 'object' },
      evidenceRequirements: ['local result text'],
      resourceRequirements: ['browser profile'],
      providerContractIds: ['provider-local-form'],
      delegatedActionBindings: bindings,
...
function bindingContract(overrides = {}) {
  return {
    sourceAction: 'run_approved_test_profile',
    sourceTarget: 'project:trainingos',
    runtimeAction: 'submit_payload',
    runtimeTarget: 'http://127.0.0.1:43119/task-form.html',
    payloadBinding: 'delegation_payload_json_v1',
    ...overrides,
  };
}
...
function readiness(overrides = {}) {
  return Object.freeze({
    ...
    actionId: 'aiexe:management-action:run-tests:26',
    actionType: 'run_approved_test_profile',
    projectId: 'trainingos',
    delegationRequestRef: 'request-26',
    destinationWorkspaceId: 'workspace-a',
    destinationExecutionBindingRef: 'binding-26',
    destinationLocalMissionRef: 'mission-26',
    destinationLocalExecutionRunRef: 'run-26',
    destinationActionTaskRef: 'task-26',
    destinationActionReadinessObservationAccepted: true,
    destinationActionReadinessState: 'waiting_human',
    destinationActionHumanGateRequested: true,
    destinationActionHumanGateState: 'requested',
...
function serviceFor({ capabilityVersion = version(), taskOverrides = {}, installationOverrides = {} } = {}) {
  const task = {
    ...
    installationId: 'install-26',
    capabilityAction: 'run_approved_test_profile',
    target: 'project:trainingos',
    state: 'waiting_human',
    ...taskOverrides,
  };
  const installation = {
...
test('M2.26 real current-version contract absence fails closed instead of silently aliasing management action to runtime action', () => {
  ...
  const result = observeA2DestinationEffectEntryPreflight({
    actionReadiness: readiness(), destinationWorkspaceId: 'workspace-a', s8Service: service,
  });
...
test('M2.26 exact destination-owned capability binding is observable but pending action HumanGate still cannot enter effect', () => {
  const capabilityVersion = version({ bindings: [bindingContract()] });
  const service = serviceFor({ capabilityVersion });
  const result = observeA2DestinationEffectEntryPreflight({
    actionReadiness: readiness(), destinationWorkspaceId: 'workspace-a', s8Service: service,
  });
  assert.equal(result.destinationEffectEntryPreflightAccepted, true);
...
test('M2.26 task action or installed-version drift fails closed before runtime binding can be trusted', () => {
  const capabilityVersion = version({ bindings: [bindingContract()] });
  const actionDrift = serviceFor({ capabilityVersion, taskOverrides: { capabilityAction: 'submit_payload' } });
  const actionResult = observeA2DestinationEffectEntryPreflight({
    actionReadiness: readiness(), destinationWorkspaceId: 'workspace-a', s8Service: actionDrift,
  });
  assert.equal(actionResult.destinationEffectEntryPreflightAccepted, false);
  assert.equal(actionResult.destinationEffectEntryPreflightReason, 'destination_action_task_management_action_drift');

  const versionDrift = serviceFor({ capabilityVersion, installationOverrides: { integrityDigest: `sha256:${'d'.repeat(64)}` } });
  const versionResult = observeA2DestinationEffectEntryPreflight({
    actionReadiness: readiness(), destinationWorkspaceId: 'workspace-a', s8Service: versionDrift,
  });
...
test('M2.26 blocked action may audit compatibility but never becomes effect-entry eligible', () => {
  const capabilityVersion = version({ bindings: [bindingContract()] });
  const service = serviceFor({ capabilityVersion });
  const result = observeA2DestinationEffectEntryPreflight({
    actionReadiness: readiness({
      destinationActionReadinessState: 'blocked',
...
test('M2.26 rejects caller runtime/effect answers before read and contains no decision or execution method', () => {
  ...
  for (const [key, value] of [
    ...
    assert.throws(() => observeA2DestinationEffectEntryPreflight({
      actionReadiness: readiness(), destinationWorkspaceId: 'workspace-a', s8Service: service, [key]: value,
    }), /unsupported field/);
  }
