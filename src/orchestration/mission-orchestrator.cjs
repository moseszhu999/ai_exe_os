...
const CONTAINED_ATTEMPT_STATES = new Set(['waiting_human', 'recovery_required']);
...
function createStepAttempt({ run, step, attemptNumber = 1, createdAt = new Date().toISOString() }) {
  ...
  return Object.freeze({
    id: semanticAttemptId(run.id, step.id, attemptNumber),
    missionRunId: run.id,
...
function transitionAttempt(attempt, nextState, reason, occurredAt = new Date().toISOString()) {
  const allowed = {
    ready: new Set(['waiting_human', 'active', 'blocked', 'cancelled']),
    waiting_human: new Set(['active', 'cancelled', 'recovery_required']),
    active: new Set(['completed', 'failed', 'recovery_required', 'waiting_human']),
    blocked: new Set(['ready', 'cancelled']),
    recovery_required: new Set(['cancelled']),
    completed: new Set(),
    failed: new Set(),
    cancelled: new Set(),
  };
...
function recoverUncertainAttempts(attempts, occurredAt = new Date().toISOString()) {
  return attempts.map((attempt) => attempt.state === 'active'
    ? Object.freeze({ ...attempt, state: 'recovery_required', recoveryReason: 'application_recovery_requires_review', updatedAt: occurredAt })
    : attempt);
}
...
function retryAfterReview({ previousAttempt, run, step, reviewed = false, occurredAt = new Date().toISOString() }) {
  ...
  if (!CONTAINED_ATTEMPT_STATES.has(previousAttempt.state) && !['failed', 'cancelled'].includes(previousAttempt.state)) {
...
function transitionRun(run, nextState, reason, occurredAt = new Date().toISOString()) {
  const allowed = {
    ...
    running: new Set(['paused', 'cancelled', 'completed', 'failed', 'recovery_required']),
...
function evaluateMissionCompletion({ run, plan, attempts, terminalEvidenceByStep = {} }) {
  ...
  for (const terminalStepId of plan.terminalStepIds) {
    const completed = completedAttempt(attempts, terminalStepId);
    if (!completed) blockers.push({ code: 'dependency_unsatisfied', terminalStepId });
    const step = plan.steps.find((candidate) => candidate.id === terminalStepId);
    const required = step?.evidenceRequirements || [];
    const available = new Set(terminalEvidenceByStep[terminalStepId] || []);
    for (const requirement of required) {
      if (!available.has(requirement)) blockers.push({ code: 'terminal_evidence_unsatisfied', terminalStepId, requirement });
    }
  }
