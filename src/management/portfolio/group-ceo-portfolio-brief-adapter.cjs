...
const ROOT_ALLOWED_KEYS = new Set([
  'schema', 'briefRef', 'observedAt', 'portfolioHealth',
  'cardCount', 'attentionCardCount', 'staleCardCount',
  'goalCount', 'opportunityCount', 'projectCount', 'exceptionCount',
  'cards', 'decisions', 'businessPerformance', 'detailIndex',
  'detailOnlyFieldsExcludedFromCards', 'readModelOnly', 'digestTraceHiddenFromPrimaryCards',
  ...ROOT_NO_AUTHORITY_FLAGS,
  'briefDigest',
]);
...
function assertCompactCard(input) {
  const card = plainObject(input, 'CEO portfolio compact card');
  ...
  if (freshness === 'stale' && (health !== 'unknown' || card.attentionRequired !== true || card.reasonCode !== 'source_stale')) {
  ...
  const decisionRef = card.decisionRef == null
    ? null
    : safeRef(card.decisionRef, 'compact card decisionRef', 'group:owner-decision:');
...
function assertDetail(input) {
  const detail = plainObject(input, 'CEO portfolio detail index row');
...
function assertDecision(input, cardByRef) {
  const decision = plainObject(input, 'CEO decision proposal');
...
function assertDecisionQueue(input, cardByRef) {
  const queue = plainObject(input, 'CEO decision queue');
  ...
  if (visibleDecisionCount !== Math.min(totalDecisionCount, DECISION_TARGET_MAX)) {
    throw new TypeError('CEO decision queue visible capacity mismatch');
  }
  if (deferredDecisionCount !== Math.max(0, totalDecisionCount - visibleDecisionCount)) {
    throw new TypeError('CEO decision queue deferredDecisionCount mismatch');
  }
  ...
  const expectedOrder = [...decisions].sort((left, right) => {
    const urgency = URGENCY_WEIGHT[left.urgency] - URGENCY_WEIGHT[right.urgency];
    return urgency !== 0 ? urgency : left.decisionRef.localeCompare(right.decisionRef);
  }).map((item) => item.decisionRef);
  if (JSON.stringify(refs) !== JSON.stringify(expectedOrder)) throw new TypeError('CEO decision queue ordering drift');
...
function assertDownstreamMetric(input) {
  if (input == null) return null;
  const metric = plainObject(input, 'business downstreamMetric');
  ...
  return freezeDeep({
    ...
    baseline: metric.baseline == null
      ? null
      : boundedNumber(metric.baseline, 'business downstreamMetric baseline', { min: -1e12, max: 1e12 }),
  });
}

function assertBusinessPerformance(input) {
  const value = plainObject(input, 'business performance row');
  ...
  for (const key of ['successRate', 'failureRate', 'unknownRate', 'humanTakeoverRate', 'errorRate', 'reversalRate']) {
    normalizedRates[key] = boundedNumber(value[key], `business performance ${key}`, { min: 0, max: 1 });
  }
...
function aggregateHealth(cards) {
...
function buildProjects(cards) {
  ...
  const projects = domains.map((ownerDomain) => {
    const domainCards = cards.filter((card) => card.ownerDomain === ownerDomain);
    return freezeDeep({
      ownerDomain,
      managementProjectId: projectIdForOwnerDomain(ownerDomain),
      health: aggregateHealth(domainCards),
  ...
  return freezeDeep(projects.sort((left, right) => left.managementProjectId.localeCompare(right.managementProjectId)));
}

function createManagementCeoPortfolioView(input) {
  const brief = plainObject(input, 'Group CEO portfolio brief');
  assertAllowedKeys(brief, ROOT_ALLOWED_KEYS, 'Group CEO portfolio brief');
  ...
  if (!['on_track', 'attention', 'blocked'].includes(portfolioHealth)) {
    throw new TypeError('Group CEO portfolio brief portfolioHealth is unsupported');
  }

  if (!Array.isArray(brief.cards) || brief.cards.length < 1 || brief.cards.length > 256) {
    throw new TypeError('Group CEO portfolio brief cards must be bounded');
  }
  ...
  if (aggregateHealth(cards) !== portfolioHealth) throw new TypeError('Group CEO portfolio brief portfolioHealth mismatch');

  if (!Array.isArray(brief.detailIndex) || brief.detailIndex.length !== cards.length) {
    throw new TypeError('Group CEO portfolio brief detailIndex mismatch');
  }
  ...
  if (JSON.stringify([...cardRefs].sort()) !== JSON.stringify([...detailRefs].sort())) {
    throw new TypeError('Group CEO portfolio brief detailIndex must cover exact card set');
  }
  ...
  for (const key of ['cardCount', 'attentionCardCount', 'staleCardCount', 'goalCount', 'opportunityCount', 'projectCount', 'exceptionCount']) {
    boundedInteger(brief[key], `Group CEO portfolio brief ${key}`, { max: 256 });
  }
  if (brief.cardCount !== cards.length) throw new TypeError('Group CEO portfolio brief cardCount mismatch');
  if (brief.attentionCardCount !== cards.filter((card) => card.attentionRequired).length) {
    throw new TypeError('Group CEO portfolio brief attentionCardCount mismatch');
  }
  if (brief.staleCardCount !== cards.filter((card) => card.freshness === 'stale').length) {
    throw new TypeError('Group CEO portfolio brief staleCardCount mismatch');
  }
  for (const [kind, field] of Object.entries({ goal: 'goalCount', opportunity: 'opportunityCount', project: 'projectCount', exception: 'exceptionCount' })) {
    if (brief[field] !== cards.filter((card) => card.cardKind === kind).length) {
      throw new TypeError(`Group CEO portfolio brief ${field} mismatch`);
    }
  }

  const decisions = assertDecisionQueue(brief.decisions, cardByRef);

  if (!Array.isArray(brief.businessPerformance) || brief.businessPerformance.length > 64) {
    throw new TypeError('Group CEO portfolio brief businessPerformance must be bounded');
  }
  ...
  if (!Array.isArray(brief.detailOnlyFieldsExcludedFromCards)
    || JSON.stringify(brief.detailOnlyFieldsExcludedFromCards) !== JSON.stringify(expectedExcluded)) {
    throw new TypeError('detail-only field boundary drift');
  }

  const primaryCards = freezeDeep([...cards]
  ...
  const ownerAttention = freezeDeep(primaryCards
  ...
  const view = {
    ...
    ownerAttention,
    decisions,
    businessPerformance: freezeDeep([...businessPerformance]
      .sort((left, right) => left.managementProjectId.localeCompare(right.managementProjectId) || left.actionCode.localeCompare(right.actionCode))),
