...
function result(actionReadiness, destinationWorkspaceId, accepted, reason, extra = {}) {
  return freezeDeep({ ...baseResult(actionReadiness, destinationWorkspaceId), destinationEffectEntryPreflightAccepted: accepted, destinationEffectEntryPreflightReason: reason, ...extra });
}
...
function observeCapabilityActionBinding({ actionReadiness, missionState }) {
  if (missionState?.activeWorkspaceId !== actionReadiness.destinationWorkspaceId) return { accepted: false, reason: 'destination_mission_state_workspace_mismatch' };
  ...
  if (task.one.workspaceId !== actionReadiness.destinationWorkspaceId) return { accepted: false, reason: 'destination_action_task_workspace_mismatch', task: task.one };
  ...
  if (installation.one.workspaceId !== actionReadiness.destinationWorkspaceId || installation.one.status !== 'installed') return { accepted: false, reason: 'destination_action_installation_not_active_in_workspace', task: task.one, installation: installation.one };
  const capabilityVersionRef = `${installation.one.packageId}@${installation.one.version}`;
  const version = exactOne(s1.marketplace, (item) => `${item.packageId}@${item.version}` === capabilityVersionRef);
  if (!version.exact) return { accepted: false, reason: 'destination_capability_version_missing_or_ambiguous', task: task.one, installation: installation.one, capabilityVersionRef };
  if (version.one.status !== 'available' || version.one.integrityDigest !== installation.one.integrityDigest) return { accepted: false, reason: 'destination_capability_version_not_exact_installed_version', task: task.one, installation: installation.one, capabilityVersionRef, version: version.one };

  let bindings;
  try { bindings = normalizeDelegatedActionBindings(version.one.delegatedActionBindings || []); }
  catch (error) { return { accepted: false, reason: 'destination_capability_action_binding_contract_invalid', detail: error.message, task: task.one, installation: installation.one, capabilityVersionRef, version: version.one }; }

  const expectedSourceTarget = actionReadiness.projectId ? `project:${actionReadiness.projectId}` : null;
  if (!expectedSourceTarget) return { accepted: false, reason: 'destination_management_source_target_unavailable', task: task.one, installation: installation.one, capabilityVersionRef, version: version.one };
  const matching = bindings.filter((binding) => binding.sourceAction === actionReadiness.actionType && binding.sourceTarget === expectedSourceTarget);
  if (matching.length === 0) return { accepted: true, reason: 'destination_capability_action_binding_missing', task: task.one, installation: installation.one, capabilityVersionRef, version: version.one, compatible: false, binding: null };
  if (matching.length !== 1) return { accepted: false, reason: 'destination_capability_action_binding_ambiguous', task: task.one, installation: installation.one, capabilityVersionRef, version: version.one };
  const binding = matching[0];
  if (task.one.capabilityAction !== binding.runtimeAction || task.one.target !== binding.runtimeTarget) {
    return { accepted: false, reason: 'destination_action_task_runtime_binding_drift', task: task.one, installation: installation.one, capabilityVersionRef, version: version.one, binding };
  }
  return { accepted: true, reason: 'destination_capability_action_binding_exact', task: task.one, installation: installation.one, capabilityVersionRef, version: version.one, compatible: true, binding };
}
...
function observeA2DestinationEffectEntryPreflight(input) {
  ...
  for (const key of Object.keys(input)) if (!allowed.has(key)) throw new Error(`M2.26 input contains unsupported field: ${key}`);
  const actionReadiness = input.actionReadiness;
  const destinationWorkspaceId = String(input.destinationWorkspaceId || '').trim();
  if (!actionReadiness || actionReadiness.schema !== A2_S8_DESTINATION_ACTION_READINESS_SCHEMA) return result(actionReadiness, destinationWorkspaceId || null, false, 'destination_action_readiness_schema_invalid');
  if (!destinationWorkspaceId) return result(actionReadiness, null, false, 'destination_workspace_required');
  if (destinationWorkspaceId !== actionReadiness.destinationWorkspaceId) return result(actionReadiness, destinationWorkspaceId, false, 'destination_workspace_mismatch');
  ...
  if (!observation.accepted) return result(actionReadiness, destinationWorkspaceId, false, observation.reason, { destinationMissionStateQueryPerformed: true, destinationCapabilityVersionObserved: !!observation.version, destinationCapabilityVersionRef: observation.capabilityVersionRef || null, effectEntryReason: observation.reason });
  if (!observation.compatible) return result(actionReadiness, destinationWorkspaceId, true, observation.reason, { destinationMissionStateQueryPerformed: true, destinationCapabilityVersionObserved: true, destinationCapabilityVersionRef: observation.capabilityVersionRef, destinationDelegatedActionBindingObserved: false, destinationDelegatedActionBindingCompatible: false, effectEntryEligible: false, effectEntryReason: observation.reason });
  const actionGatePending = actionReadiness.destinationActionReadinessState === 'waiting_human' && actionReadiness.destinationActionHumanGateRequested === true && actionReadiness.destinationActionHumanGateState === 'requested';
  const reason = actionGatePending ? 'destination_runtime_binding_exact_action_human_gate_pending' : 'destination_runtime_binding_exact_action_not_ready';
...
module.exports = { A2_S8_DESTINATION_EFFECT_ENTRY_PREFLIGHT_SCHEMA, observeCapabilityActionBinding, observeA2DestinationEffectEntryPreflight };
