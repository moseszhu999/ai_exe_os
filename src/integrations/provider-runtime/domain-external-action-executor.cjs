...
const { ProviderExecutionAttemptClaimGate } = require('./persistent-attempt-claim.cjs');

const DOMAIN_EXTERNAL_ACTION_EXECUTOR_VERSION = 'provider.domain-external-action-executor.v1';
const DOMAIN_EXTERNAL_ACTION_EVIDENCE_SCHEMA = 'provider.domain-external-action.evidence.v1';
...
function normalizeJson(value, label, depth = 0, forbidden = FORBIDDEN_DATA_KEY) {
  ...
  if (Array.isArray(value)) {
    return value.map((entry, index) => normalizeJson(entry, `${label}[${index}]`, depth + 1, forbidden));
  }
...
function boundedJson(value, label, maxBytes = MAX_RESPONSE_BYTES, forbidden = FORBIDDEN_DATA_KEY) {
...
function noSensitiveResponse(value, label = 'domain provider response', depth = 0) {
  if (depth > 20) throw new Error(`${label} exceeds maximum nesting depth`);
  if (Array.isArray(value)) {
    value.forEach((entry, index) => noSensitiveResponse(entry, `${label}[${index}]`, depth + 1));
    return;
  }
  if (value && typeof value === 'object') {
    for (const [key, entry] of Object.entries(value)) {
      if (FORBIDDEN_RESPONSE_KEY.test(key)) throw new Error(`${label} contains sensitive field: ${key}`);
      noSensitiveResponse(entry, `${label}.${key}`, depth + 1);
    }
    return;
  }
...
function normalizePlan(plan) {
  const input = object(plan, 'provider adapter plan');
  if (input.schema !== PROVIDER_ADAPTER_PLAN_SCHEMA) {
    throw new Error(`Unsupported provider adapter plan schema: ${input.schema}`);
  }
  const suppliedPlanDigest = sha(input.planDigest, 'plan digest');
  const base = { ...input };
  ...
  if (input.semanticOperation?.riskClass !== 'externalAction') throw new Error('P5 domain executor requires riskClass=externalAction');
  if (input.semanticOperation?.humanGatePolicy !== 'action') throw new Error('P5 domain executor requires action-level Human Gate policy');
  ...
  const providerManifestDigest = sha(input.providerManifestDigest, 'plan provider manifest digest');
  const requestId = assertSafeIdentifier(input.requestId, 'plan request id');
  const requestDigest = sha(input.requestDigest, 'plan request digest');
  const operationId = assertSafeIdentifier(input.semanticOperation.operationId, 'plan operation id');
  const targetRef = opaque(input.semanticOperation.targetRef, 'plan targetRef', 'target');

  const transport = object(input.transportBinding, 'plan transport binding');
  if (transport.mode !== 'https') throw new Error('P5 domain executor requires https transport binding');
  const endpointRef = opaque(transport.endpointRef, 'plan endpointRef', 'endpoint');
  const networkPolicyRef = opaque(transport.networkPolicyRef, 'plan networkPolicyRef', 'network');
  ...
  const credentialRef = opaque(transport.credentialRefs[0], 'plan credentialRef', 'credential');

  const call = object(input.protocolCall, 'plan protocol call');
  exactKeys(call, new Set(['callKind', 'protocolOperation', 'payload']), 'plan protocol call');
  ...
  const payload = object(call.payload, 'plan domain payload');
  exactKeys(payload, new Set(['parameters']), 'plan domain payload');
  const parameters = boundedJson(object(payload.parameters, 'plan domain parameters'), 'plan domain parameters');

  const flags = object(input.flags, 'plan flags');
  exactKeys(flags, new Set([
  ...
  return deepFreeze({
    ...
    transportBinding: deepFreeze({
      mode: 'https',
      endpointRef,
      credentialRefs: Object.freeze([credentialRef]),
      networkPolicyRef,
    }),
    protocolCall: deepFreeze({
      callKind: 'domain_operation',
      protocolOperation,
      payload: deepFreeze({ parameters: deepFreeze(structuredClone(parameters)) }),
    }),
    flags: deepFreeze({
...
function authorize(plan, authorizationRequest, at) {
  const request = object(authorizationRequest, 'execution authorization request');
...
async function resolveEndpoint(plan, resolver) {
  if (!resolver || typeof resolver.resolve !== 'function') throw new TypeError('endpointResolver.resolve is required');
  const binding = object(await resolver.resolve({
  ...
  }), 'resolved domain endpoint binding');
  const expected = {
    providerId: plan.providerId,
    operationId: plan.semanticOperation.operationId,
    providerOperation: plan.protocolCall.protocolOperation,
...
async function resolveCredential(plan, resolver) {
  if (!resolver || typeof resolver.resolve !== 'function') throw new TypeError('credentialResolver.resolve is required');
  ...
  const binding = object(await resolver.resolve({
    ...
    targetRef: plan.semanticOperation.targetRef,
    credentialRef,
  }), 'resolved domain credential binding');
  ...
  return Object.freeze({ credentialRef, secret });
}

function normalizeAdapter(plan, domainAdapter) {
  const adapter = object(domainAdapter, 'domain operation adapter');
  exactKeys(adapter, new Set([
    'adapterRef',
    'providerId',
    'providerContractId',
    'protocolFamily',
    ...
    'wireMethod',
    'credentialScheme',
    'providerSideIdempotency',
    'automaticRetry',
    'businessOutcomeAuthority',
    'invokeOperation',
  ]), 'domain operation adapter');
  const adapterRef = opaque(adapter.adapterRef, 'domain adapterRef', 'adapter');
  const expected = {
    providerId: plan.providerId,
    providerContractId: plan.providerContractId,
    protocolFamily: 'http.json',
...
function normalizeResponse(raw) {
  const response = object(raw, 'domain operation adapter response');
  exactKeys(response, new Set(['statusCode', 'contentType', 'bodyText', 'providerRequestId']), 'domain operation adapter response');
  ...
  payload = boundedJson(payload, 'domain operation response payload', MAX_RESPONSE_BYTES, FORBIDDEN_RESPONSE_KEY);
  noSensitiveResponse(payload);
  ...
  return Object.freeze({ statusCode: response.statusCode, payload, providerRequestId });
}

function receiptFor({ plan, decision, endpoint, response, startedAt, completedAt }) {
...
function domainActionEvidenceFor({ plan, attempt, adapter, receipt }) {
  const core = {
    schema: DOMAIN_EXTERNAL_ACTION_EVIDENCE_SCHEMA,
...
function finishTime(clock, fallback) {
  try {
...
async function executePersistedDomainExternalActionAttempt({
  ...
  const decision = authorize(normalizedPlan, authorizationRequest, executionAt);
  ...
  try {
    ...
    const completedAt = finishTime(clock, startedAt);
    ...
    const domainActionEvidence = domainActionEvidenceFor({
      plan: normalizedPlan,
      attempt,
      adapter,
      receipt,
