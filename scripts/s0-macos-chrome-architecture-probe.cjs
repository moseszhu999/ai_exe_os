...
function recommendation(attempts, hostArchitecture) {
  ...
  if (hostArchitecture === 'arm64' && currentChannel?.processArch === 'x64') {
    return {
      code: 'CURRENT_NODE_IS_X64_OR_TRANSLATED',
      node: process.execPath,
      reason: 'The probe is running under x64/Rosetta on an arm64 host; reinstalling Chrome is not the first remedy.',
    };
  }
...
function parent() {
  ...
  const hostArchitecture = command('/usr/bin/uname', ['-m']);
