...
async function launchInstance({ label, userData, testPort, endpoint, audit }) {
  ...
  return { app, page, bridge };
}

async function querySync(instance) {
  return instance.page.evaluate(() => window.aiExecutionOS.s7.sync.queryState('workspace-a'));
}

async function queryS0(instance) {
  return instance.page.evaluate(() => window.aiExecutionOS.getState());
}
...
async function stopWorker(instance, workerId) {
  return instance.page.evaluate((id) => window.aiExecutionOS.stopWorker(id), workerId);
}

async function push(instance) {
  return instance.page.evaluate(() => window.aiExecutionOS.s7.sync.pushPending({ workspaceId: 'workspace-a' }));
}

async function pull(instance) {
  return instance.page.evaluate(() => window.aiExecutionOS.s7.sync.pullMirror({ workspaceId: 'workspace-a' }));
}
...
async function main() {
  ...
  try {
    ...
    assert.ok(sharedA);
    const aPresence = sharedA.records.find((item) => item.recordClass === 'worker-presence.summary');
    assert.ok(aPresence);
    assert.equal(aPresence.payload.workerPublicId, 's1-worker-chrome');
    ...
    assert.ok(sharedB);
    const bPresence = sharedB.records.find((item) => item.recordClass === 'worker-presence.summary');
    assert.ok(bPresence);
    assert.equal(bPresence.payload.workerPublicId, 's1-worker-chromium');
    ...
    const result = {
      ...
      rows: {
        ...
        realChromeWorkerA: 'PASS', realChromiumWorkerB: 'PASS', bidirectionalSync: 'PASS', membershipVisibility: 'PASS',
        remoteWorkerPresenceReadOnly: 'PASS', crossInstanceLocalControlIsolation: 'PASS', noBrowserSubmission: 'PASS',
    ...
    try { await closeInstance(a); } catch {}
    try { await closeInstance(b); } catch {}
